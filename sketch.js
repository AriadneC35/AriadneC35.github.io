...
jogador = [0, 3, 10,130];     // parâmetro = [Origem(0), velocidade(1), diâmetro(2),vida(3)];
...
// Variáveis de sons
var intro = 0, durante_jogo = 0, final = 0;
var final_run = 0;            // Flag de som
...
function preload(){
  ...
  durante_jogo = loadSound("./sounds/game.mp3");
  final = loadSound("./sounds/final1.mp3");
  /*---------------------------------------------------------------SOM---------------------------------------------------------------*/
...
function modoOperacional(){
  switch(tela){
    ...
    case 1:
      intro.stop();
        if(durante_jogo.isPlaying() === false){
         durante_jogo.play();
        }
      jogo();
    break;
